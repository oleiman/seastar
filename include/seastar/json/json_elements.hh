...
public:
    ...
    explicit json_base_element(bool set) noexcept
      : _mandatory(false)
      , _set(set) {}
     ...
     */
    virtual bool is_verify() noexcept { return !(_mandatory && !_set); }
...
public:
    explicit json_element(T&& val) noexcept
      : json_base_element(true)
    ...
    json_element& operator=(T&& new_value) {
...
public:
     ...
     */
    void push(const_reference element) { push_back(element); }
    ...
    void push_back(T&& element) {
        _set = true;
        _elements.push_back(std::forward<T>(element));
    }

    /**
     * Move an element onto the list. Enabled iff T is move constructible.
     * @param element a new element that will be added to the list
     */
    template<
      class U = T,
      class = std::enable_if_t<
        std::is_same_v<T, U> && std::is_move_constructible_v<U>>>
...
struct json_base : public jsonable {
     ...
     */
    virtual void
    add(json_base_element* element, std::string name, bool mandatory = false);
...
struct json_void : public jsonable {
     ...
     * \brief write to an output stream
     */
    virtual future<> write(output_stream<char>& s) const { return s.close(); }
};
...
struct json_return_type {
    ...
    json_return_type(
      std::function<future<>(output_stream<char>&&)>&& body_writer)
...
/*!
 * \brief capture a range and return a serialize function for it as a json
 * array.
 *
 * To use it, pass a range and a mapping function.
 * For example, if res is a map:
 *
 * return make_ready_future<json::json_return_type>(stream_range_as_array(res,
 * [](const auto&i) {return i.first}));
 */
template<typename Container, typename Func>
SEASTAR_CONCEPT(requires requires(Container c, Func aa, output_stream<char> s) {
    { formatter::write(s, aa(*c.begin())) } -> std::same_as<future<>>;
})
std::function<future<>(output_stream<char>&&)> stream_range_as_array(
  Container val, Func fun) {
    return [val = std::move(val),
            fun = std::move(fun)](output_stream<char>&& s) mutable {
        return do_with(
                    ...
                    return do_for_each(
                      ...
                      [&s, &first, &f](
                        const typename Container::value_type& v) {
                          auto fut = first ? make_ready_future<>()
                                           : s.write(", ");
                          first = false;
    ...
    return [val = std::move(val)](output_stream<char>&& s) mutable {
        return do_with(
          ...
          [](output_stream<char>& s, const T& val) {
              return formatter::write(s, val).finally(
                [&s] { return s.close(); });
          });
    };
}
